...
def plot(data1, data2, title):
    ...
    for i in range(2):
        ...
        # ax.axvline(17, color="k", linewidth=2, zorder=3)
        ax.fill_between([15.5, 17.5], -30, 40, facecolor='dodgerblue', alpha=0.4, zorder=0)
        ##########################
        if i == 0:
            # ax.text(17, 37, "1999(drought turning point)", c='k', fontsize=15)
            # 渐变色柱状图
            # 归一化
            norm = plt.Normalize(-20, 20)  # 值的范围
            norm_values = norm(data1)
            cmap = cmaps.MPL_PRGn_r
            ...
            ax.set_ylabel("units: mm/month", fontsize=20)
            ax.set_ylim(0, 40)

            ax.set_xticks(t2[::3])
            ax.set_xticklabels(tt[::3])
            ax.set_title("(c) WaterYield(P-E)", loc="left", fontsize=20)
        ##########################
        
        elif i == 1:
            # ax.text(17, 37, "1999(drought turning point)", c='k', fontsize=20)
            ax = axes[i]
            # 渐变色柱状图
            # 归一化
            norm = plt.Normalize(-20, 20)  # 值的范围
...
pre_ave = mask(130, pre_gs)
print(np.nanmax(pre_ave), np.nanmin(pre_ave))


plot(pre_ave, pre_ave, f"MG Grassland Water Yield")
