...
def tmp_xarray(band1):
    t = np.arange(40)
    mn = np.arange(1, 13, 1)
    tmp = xr.DataArray(band1, dims=['mn', 't', 'y', 'x'], coords=[
                       mn, t, lat2, lon2])
...
def CreatNC(data):
    new_NC = nc.Dataset(
        r"E:/CRU/VAP_DATA/vpd_CRU_MONTH_81_20.nc", 'w', format='NETCDF4')
...
def CreatNC2(data):
    new_NC = nc.Dataset(
        r"E:/CRU/VAP_DATA/vpd_anomaly_CRU_MONTH_81_20.nc", 'w', format='NETCDF4')
...
# %%
def anomaly(data):
    data_anom = np.zeros((12, 40, 360, 720))
    data_ave = np.nanmean(data, axis=1)
    for mn in range(12):
        for yr in range(40):
            for r in range(360):
                for c in range(720):
                    data_anom[mn, yr, r, c] = data[mn, yr, r, c]-data_ave[mn, r, c]
                    
    return data_anom

vpd_anom = anomaly(vpd)
CreatNC2(vpd_anom)
