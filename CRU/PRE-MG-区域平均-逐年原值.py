...
def plot(data1, data2, title):
    ...
    for i in range(2):
        ...
        ##########################
        if i == 0:
            ax.text(17, 94, "1999(drought turning point)", c='k', fontsize=15)
            # 渐变色柱状图
            # 归一化
            norm = plt.Normalize(-100, 100)  # 值的范围
            ...
            ax.set_title("PRE", loc="left", fontsize=15)
        ##########################
        elif i == 1:
            ax.text(17, 94, "1999(drought turning point)", c='k', fontsize=15)
            ax = axes[i]
...
plot(pre_ave, pre_ave, f"MG Grassland PRE Original")
