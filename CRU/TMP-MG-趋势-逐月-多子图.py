# -*- coding: utf-8 -*-
"""
Created on Tue Oct 25 11:03:38 2022
...
def read_nc():
    global ndvi, lat, lon
    inpath = r"E:/GIMMS_NDVI/data_RG/NDVI_82_15_RG.nc"
...
def read_nc2():
    global s, p
    inpath = "E:/GIMMS_NDVI/data_RG/NDVI_82_15_RG_Trend.nc"
    with nc.Dataset(inpath) as f:
        #print(f.variables.keys())
        s = (f.variables['s'][:])
        p = (f.variables['p'][:])
...
def ndvi_xarray(band1):
    t = np.arange(34)
    ndvi=xr.DataArray(band1,dims=['t','y','x'],coords=[t, lat, lon])
    return ndvi.loc[:, 55:40, 100:125]
...
#%%
read_nc()
ndvi_mn = np.nanmean(ndvi, axis=0)
def read_plot(data1):
    '''
    global data
    data = ndvi_xarray(data1)
    '''
    print('5 percentile is: ' , np.nanpercentile(data1, 5))
    print('95 percentile is: ' , np.nanpercentile(data1, 95), "\n")
    
    levels = np.arange(0, 0.91, 0.05)
    
    cmap = cmaps.NOC_ndvi
    data2 = np.ones((12, 300, 600))
    Creat_figure(data1, data2, levels, cmap, lat, lon, title=f"MG NDVI Monthly Mean 82-15")
...
def read_plot2(data1, data2):
    ...
    Creat_figure(data1, data2, levels, cmap, lat, lon, title=f"MG NDVI Monthly Trend 82-15")
