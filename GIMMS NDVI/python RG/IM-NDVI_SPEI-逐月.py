...
def plot(data1, data2, title):
    ...
    for i in range(4):
        for j in range(3):
            ...
            lin1 = ax2.scatter(t2, data2[:, ind], color='Green', label="NDVI", alpha=0.5) #
            ...
            lin2 = axs[i, j].bar(t, data1[ind, :], color=colors, label="SPEI03", zorder=2) #
            ...
            '''
            ax2.set_ylim(-2, 2)
            ax2.set_yticks(np.arange(-1.8, 1.81, 0.3))
...
plot(spei_mn_ave, ndvi_IM_z_ave, f"MG Grassland SPEI_NDVI")