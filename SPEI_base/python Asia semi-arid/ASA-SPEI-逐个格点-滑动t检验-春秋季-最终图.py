...
def read_nc():
    ...
    with nc.Dataset(inpath) as f:
        ...
        spei2 = np.nanmean(spei[[1, 2],], axis=0)
    return spei2
...
def CreatMap(data1, lon, lat, levels, colors, title):
    ...
    ax.text(71, 57, "(a)", fontsize=15, transform=ccrs.PlateCarree())
    
    # select 矩形关键区区域
    RE = Rectangle((100, 40), 25, 15, linewidth=2, linestyle='-', zorder=3,
    ...
    plt.savefig(
        rf'E:/SPEI_base/python Asia semi-arid/JPG GS/{title}.jpg', bbox_inches='tight')
    plt.show()
...
def yr_cut(N, tt, spei, sn):
    ...
    for i, x in enumerate(num.index[2:5]):
        title = f"Moving T-test Number {x} (n=10) {sn}"
...
def legend(fig, ax):
    ...
    labels = [x+1 for x in range(5)]
    sub_ax.legend(rectangles, labels, loc="upper right",
              bbox_to_anchor=(1, 1), fancybox=True, frameon=True,
              fontsize=12, ncol=2, title="Number of Turning Point")
    # ax.spines['left'].set_visible(False)
    # plt.savefig(
    #     rf'E:/SPEI_base/python Asia semi-arid/JPG GS/legend.jpg', bbox_inches='tight')
    # plt.show()
...
def region1(data):
    yr = np.arange(40)
    spei_global = xr.DataArray(data, dims=['yr', 'y', 'x'], coords=[
                               yr, lat, lon])  # 原SPEI-base数据
...
def bar(data, title):
    fig = plt.figure(figsize=(6, 3), dpi=1080)
    ax = fig.add_subplot(111)
    year = np.arange(1996, 2005)
    ax.bar(year, data, color='b', alpha=0.5, width=0.8)
    ax.tick_params(labelsize=15)
    ax.set_xticks(year)
    ax.set_yticks([0, 0.2, 0.4, 0.6])
    ax.set_yticklabels(["0", "20%", "40%", "60%"])
    ax.set_ylabel("Area Percentage", fontsize=15)
    ax.text(0.1, 0.8, "(b)", fontsize=15)
...
year_count = yr_cut2(N, tt, spei_MG, season)
bar(year_count, f"{season}")
print(f"{season} is done!")
