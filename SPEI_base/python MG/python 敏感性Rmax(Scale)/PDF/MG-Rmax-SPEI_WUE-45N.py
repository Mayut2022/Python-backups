# -*- coding: utf-8 -*-
"""
Created on Fri Nov 18 22:45:39 2022
...
def sns_plot(df_g, title):
    ...
    for i in range(4):
        for j in range(3):
            hist_kws = dict(color="#7B68EE", alpha=0.5)
    ...
    for i in range(4):
        for j in range(3):      
            ...
            if ave==max(mean):
                ...
                axes[i, j].axvline(x=ave, c='blue')
                axes[i, j].text(ave+0.02, 130, f"Mean={ave:.2f}", color="blue", fontsize=15)
            else:
...
for mn in month:
    ...
    for s in scale:
        ...
        r, p, _, df = corr(wue_mn, spei_mn, df, s)
        print(f"scale spei{s} is done!")
        
    print(f"Next is sns plot-----")
    df_g = df[df["LCC"]==130]
    ##### 以45N为界
    df_g_N = df_g[df_g["LAT"]>45]
    df_g_S = df_g[df_g["LAT"]<45]
    sns_plot(df_g_N, title=f"WUE-SPEI 45N North CORR 82-18 {mn_str[mn-1]}")   
    sns_plot(df_g_S, title=f"WUE-SPEI 45N South CORR 82-18 {mn_str[mn-1]}")    