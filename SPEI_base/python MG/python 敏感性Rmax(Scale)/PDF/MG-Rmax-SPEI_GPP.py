# -*- coding: utf-8 -*-
"""
Created on Fri Nov 18 22:33:04 2022
...
def sns_plot(df_g, title):
    ...
    for i in range(4):
        for j in range(3):
            hist_kws = dict(color="orange", alpha=0.5)
...
for mn in month:
    ...
    for s in scale:
        ...
        r, p, _, df = corr(gpp_mn, spei_mn, df, s)
        print(f"scale spei{s} is done!")
        
    print(f"Next is sns plot-----")
    df_g = df[df["LCC"]==130]
    sns_plot(df_g, title=f"GPP-SPEI CORR 82-18 {mn_str[mn-1]}")    