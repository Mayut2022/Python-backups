...
def read_nc2():
    ...
    with nc.Dataset(inpath) as f:
        ...
        e_diff = e_gs[1:, ]-e_gs[:-1, ]
        e_ave = mask(130, e_diff)
    return e_ave
...
def plot(data1, data2, title):
    ...
    for i in range(2):
        ...
        ##########################
        if i == 0:
            ax.text(16, 8.5, "1999(drought turning point)", c='b', fontsize=15)
            # 渐变色柱状图
            # 归一化
            ...
            ax.set_title("ET Interannual increment", loc="left", fontsize=15)
        ##########################
        elif i == 1:
            ax.text(16, 8.5, "1999(drought turning point)", c='b', fontsize=15)
            ax = axes[i]
            ...
            ax.set_ylabel("units: mm/month", fontsize=15)
            ax.set_ylim(-10, 10)
            ax.set_title("T Interannual increment", loc="left", fontsize=15)
    # label
...
# %%
plot(e_ave, et_ave, title=r"MG Grassland ET&T interannual increment")