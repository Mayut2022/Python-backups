...
# %% SPEI\T\CO2 按平均值数据进行试验

def data_ave(data):
    data_gs = np.empty((37, 30, 50))
    for yr in range(37):
        data_gs[yr, :, :] = np.nanmean(data, axis=0)

    return data_gs


for x in var0:
    exec(f"{x}_00 = data_ave({x}_gs)")
    sos_00 = data_ave(sos)
...
def CreatNC():
    new_NC = nc.Dataset(
        r"E:/CO2/python-AGU-MG/FittingMG_SensitivityC5.nc", 'w', format='NETCDF4')
    ...
    var.description = "fit_all原数据拟合，\
            fit_()00为X变量的平均值敏感性试验得出来的拟合值，X_sen为其敏感性系数"
