...
def mask_sig(data, data_m):
    data_mask = ma.masked_array(data, mask=data_m>0.05)
    data_g = mask_lcc(130, data_mask)
...
del lat2, lon2
#########统一年份
gpp_y = gpp[1:33, 5:8, :, :].mean(axis=1)

pre = pre[3:6, 1:33, :, :].mean(axis=0)
co2 = co2[5:8, 1:, :, :].mean(axis=0)
gpp0 = gpp[1:33, 4:7, :, :].mean(axis=1)
...
params = np.zeros((4, 30, 50))
fit_tp = np.zeros((4, 30, 50))
...
for r in range(30):
    for c in range(50):
        ...
        if True in df.isnull().values:
            ...
            params[:, r, c] = fit.params ####依次为截距、各变量的参数
            fit_tp[:, r, c] = fit.pvalues

#%% mask 看草地
fit_r2_mk = mask_sig(fit_r2, fit_fp)

params_mk = np.zeros((4, 30, 50))
params_mk = np.ma.array(params_mk)

for i in range(3):
    params_mk[i, :, :] = mask_sig(params[i, :, :], fit_tp[i, :, :])
    x = params_mk[i, :, :]
    
#%%
print(fit_r2_mk.mean())
