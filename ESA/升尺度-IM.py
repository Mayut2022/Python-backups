# -*- coding: utf-8 -*-
"""
Created on Tue Oct 18 19:12:33 2022
...
#%%
def down_scaling():
    lat_spei = np.arange(37., 46.1, 0.5) ########
    lon_spei = np.arange(105, 125.1, 0.5) ########
    spei_lcc = np.zeros((18, 40)) ########
...
spei_lat = np.arange(37.25, 46, 0.5) ########
spei_lon = np.arange(105.25, 125, 0.5) ########
#%%
lcc, lat, lon = read_nc(46, 37, 105, 125) ########
#%%
spei_lcc = down_scaling()
df = lcc_count(lcc, df)

#%%
writer = pd.ExcelWriter("IM ESA LUC降尺度SPEI03.xlsx", engine='xlsxwriter')

df.to_excel(writer, sheet_name="IM", index=False)
    
writer.save()
#%% 存储NC文件
def CreatNC(data):
    new_NC = nc.Dataset(r"E:\ESA\ESA_LUC_2000_SPEI03_IM.nc", 'w', format='NETCDF4')
    
    new_NC.createDimension('lat', 18) ########
    new_NC.createDimension('lon', 40) ########
