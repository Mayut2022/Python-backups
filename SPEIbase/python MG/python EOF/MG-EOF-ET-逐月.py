# -*- coding: utf-8 -*-
"""
Created on Sun Nov 20 14:35:54 2022
...
def read_plot(data1, data2):
    ...
    cmap = "RdBu"
    Creat_figure(data1, data2, levels, cmap, lat, lon, title=f"Grassland ET EOF {mn_str[mn-1]}")
...
e_MG = read_nc()
read_nc3()
e_MG_g = mask(130, e_MG)

month = np.arange(4, 11, 1) ## Apr-Oct 生长季

for mn in month:
    print(f"month {mn} is in programming!")
    eof, pc, percentContrib, test = eof_cal(e_MG_g[mn-1, 1:38, :, :])
