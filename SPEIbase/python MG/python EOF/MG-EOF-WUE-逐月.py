# -*- coding: utf-8 -*-
"""
Created on Sat Nov 19 21:18:44 2022
...
def Creat_figure(data1, data2, levels, cmap, lat, lon, title):
    ...
    for j in range(4, 7):
        ...
        ax.set_title(f"PC0{j-3} ({percentContrib[j-4]:.2f}%)", fontsize=15, loc="left")
    ...
    ax.text(2025, -7, f"{test}", color="dimgray")
    
    plt.suptitle(f'{title}', fontsize=25)
    plt.savefig(rf'E:/SPEI_base/python MG/JPG/WUE/EOF/{title}.jpg', bbox_inches = 'tight')
    plt.show()
...
def read_plot(data1, data2):
    ...
    levels = np.arange(-0.03, 0.031, 0.003)
    cmap = cmaps.MPL_PRGn
    Creat_figure(data1, data2, levels, cmap, lat, lon, title=f"Grassland WUE EOF {mn_str[mn-1]}")
...
wue_MG = exact_data3()
read_nc3()
wue_MG_g = mask(130, wue_MG)

month = np.arange(4, 11, 1) ## Apr-Oct 生长季

for mn in month:
    print(f"month {mn} is in programming!")
    eof, pc, percentContrib, test = eof_cal(wue_MG_g[:, mn-1, :, :])
