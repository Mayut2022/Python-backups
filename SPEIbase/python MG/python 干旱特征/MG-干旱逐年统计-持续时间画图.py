...
def drought_duration(data):
    ...
    data_dd = data_df[np.logical_and(
        data_df < -0.5, data_df >= -1)].dropna(axis=1, how='all').count()
    dd1 = data_dd.sum()/len(data_dd)
    data_dd2 = data_df[np.logical_and(
        data_df < -1, data_df >= -1.5)].dropna(axis=1, how='all').count()
    dd2 = data_dd2.sum()/len(data_dd2)
    data_dd3 = data_df[np.logical_and(
        data_df < -1.5, data_df >= -2)].dropna(axis=1, how='all').count()
    dd3 = data_dd3.sum()/len(data_dd3)
    data_dd4 = data_df[data_df < -2].dropna(axis=1, how='all').count()
    dd4 = data_dd4.sum()/len(data_dd4)
    ...
    # 计算累计干旱强度
    # ds = data_df[np.logical_and(data_df<-0.5, data_df>=-1)].sum()
...
df_dd = pd.DataFrame()
for yr in range(40):
    dd_yr = drought_duration(spei_mn[yr, :, ])
...
def area_plot(data, data2, title):
    ...
    ax.text(18, 10.5, "1999(turning point)", c='b')
    # linewidth控制间隙
    data.plot.bar(
    ...
    ax.set_title(f"Drought Duration", fontsize=10, loc="left")
    ...
    ax2.set_title(f"Drought Duration Comparison", fontsize=10, loc="left")
...
area_plot(df_dd.iloc[:, ::-1], dd_cp, title=r"MG Grassland Drought Duration")
