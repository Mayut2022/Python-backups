...
def read_nc():
    ...
    with nc.Dataset(inpath) as f:
        ...
        spei1 = spei[:, 9:, :]
        spei2 = spei[1:, :4, ]
        spei40 = np.array([np.nan]*6000).reshape(1, 4, 30, 50)
        
        spei2 = np.vstack((spei2, spei40))
        
        spei1 = spei1.swapaxes(0, 1)
        spei2 = spei2.swapaxes(0, 1)
        
        spei_ngs = np.vstack((spei1, spei2)).reshape((40*7, 30, 50), order="F")
    return spei_ngs
...
def t_xaxis():
    ...
    for i, yr in enumerate(year):
        t = pd.date_range(f'{yr}/10', periods=7, freq="MS")
        tt = []
...
def plot(data1, title):
    ...
    ax.axvline(133, color="b", linewidth=1, zorder=3)
    ax.text(133, 2., "1999(turning point)", c='b')
    ...
    ax.set_title(f"1981-2020 NGS SPEI03", fontsize=10, loc="left")
    ax.set_ylim(-2.5, 2.5)
    ax.set_yticks(np.arange(-2, 2.1, 1))
    ax.tick_params(labelsize=10)
    ax.set_xlim(-10, 290)
    ax.set_xticks(t[::14])
    ax.set_xticklabels(t_date[::14], rotation=60)
...
plot(spei_mk, title=r"Grassland 1981-2020 NGS SPEI03")
