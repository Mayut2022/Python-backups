...
def Creat_figure(data1, data2, data3, levels, cmap, lat, lon, title):
    ...
    plt.savefig(
        rf'E:/GLASS-GPP/JPG MG/Comparison2/{title}.jpg', bbox_inches='tight')
    plt.show()
...
def PLOT2(data1, data2, lev):
    ...
    lev2 = lev.split('<')[1]
    # title = rf"Drought GPPZ (SPEI under {lev2})"
    title = rf"Non-Drought GPPZ (SPEI under {lev2})"
    print("\n", title)
...
# %%
lev1 = "(spei)<-1"
gNm1, _, gYm1, gYc1 = gpp_drought(lev1, gpp1, spei1)
gNm2, _, gYm2, gYc2 = gpp_drought(lev1, gpp2, spei2)
