# -*- coding: utf-8 -*-
"""
Created on Sun Feb 26 12:01:10 2023

@author: MaYutong
"""
import warnings
import cmaps
import cartopy.crs as ccrs
import cartopy.feature as cfeature
from cartopy.mpl.gridliner import LONGITUDE_FORMATTER, LATITUDE_FORMATTER
from cartopy.mpl.ticker import LongitudeFormatter, LatitudeFormatter
from cartopy.io.shapereader import Reader, natural_earth

import matplotlib.pyplot as plt
import netCDF4 as nc
import numpy as np
from scipy import stats
import xarray as xr

plt.rcParams['font.sans-serif'] = ['times new roman']  # 指定默认字体
warnings.filterwarnings("ignore")

# %%


def read_nc():
    global lat, lon
    # global a1, a2, o1, o2
    inpath = (r"E:/ESA/ESA_LUC_2000_SPEI03_MG.nc")
    with nc.Dataset(inpath) as f:
        # print(f.variables.keys())
        # print(f.variables['lccs_class'])
        lat = (f.variables['lat'][:])
        lon = (f.variables['lon'][:])
        lcc = (f.variables['lcc'][:])
    return lcc

# %%


def mn_yr(data):
    tmp_mn = []
    for mn in range(12):
        tmp_ = []
        for yr in range(40):
            tmp_.append(data[mn])
            mn += 12
        tmp_mn.append(tmp_)

    tmp_mn = np.array(tmp_mn)

    return tmp_mn


def MG(data):
    t = np.arange(1, 481, 1)
    l = np.arange(1, 5, 1)
    spei_global = xr.DataArray(data, dims=['t', 'l', 'y', 'x'], coords=[
                               t, l, lat2, lon2])  # 原SPEI-base数据
    spei_rg1 = spei_global.loc[:, :, 40:55, 100:125]
    spei_rg1 = np.array(spei_rg1)
    return spei_rg1

# %%


def read_nc2():
    global lat2, lon2
    inpath = rf"E:/GLDAS Noah/DATA_RG/SM_81_20_ORINGINAL_SPEI0.5x0.5.nc"
    with nc.Dataset(inpath, mode='r') as f:
        # print(f.variables.keys())
        # print(f.variables['sm'])
        sm = (f.variables['sm'][:, :, :])
        lat2 = (f.variables['lat'][:])
        lon2 = (f.variables['lon'][:])
    sm_MG = MG(sm)
    sm_MG_mn = mn_yr(sm_MG)
    sm_gsl = np.nanmean(sm_MG_mn[4:9, :], axis=0)
    return sm_gsl

# %%


def read_nc3():
    inpath = rf"E:/GLDAS Noah/DATA_RG/SM_Anom_81_20_ORINGINAL_SPEI0.5x0.5.nc"
    with nc.Dataset(inpath, mode='r') as f:
        # print(f.variables.keys())
        sm_a = (f.variables['sm'][:])
    sm_a_MG = MG(sm_a)
    sm_MG_mn = mn_yr(sm_a_MG)
    sm_gsl = np.nanmean(sm_MG_mn[4:9, :], axis=0)
    return sm_gsl


# %%
def read_nc4():

    def read_smz(inpath):
        with nc.Dataset(inpath, mode='r') as f:
            # print(f.variables.keys())
            sm_z = (f.variables['sm_z'][:])
        return sm_z

    for i in range(1, 13):
        inpath = rf"E:/GLDAS Noah/DATA_RG/Zscore_Anomaly/SM_81_20_SPEI0.5x0.5_Zscore_Anomaly_month{i}.nc"
        sm_z = read_smz(inpath)
        if i == 1:
            sm_z_all = sm_z
        else:
            sm_z_all = np.vstack((sm_z_all, sm_z))

    sm_z_all_MG = MG(sm_z_all)
    sm_z_all_MG = sm_z_all_MG.reshape(12, 40, 4, 30, 50)
    sm_gsl = np.nanmean(sm_z_all_MG[4:9, :], axis=0)

    return sm_gsl


# %%


def tTest(list1, list2):
    t = np.zeros((4, 30, 50))
    pt = np.zeros((4, 30, 50))

    for l in range(4):
        for r in range(30):
            for c in range(50):
                a = list1[:, l, r, c]
                b = list2[:, l, r, c]
                if np.isnan(a).any() or np.isnan(b).any():
                    t[l, r, c] = np.nan
                    pt[l, r, c] = np.nan
                else:
                    levene = stats.levene(a, b, center='median')
                    if levene[1] < 0.05:
                        t[l, r, c] = np.nan
                        pt[l, r, c] = np.nan
                    else:
                        tTest = stats.stats.ttest_ind(a, b, equal_var=True)
                        t[l, r, c] = tTest[0]
                        pt[l, r, c] = tTest[1]

    return t, pt


# %%


def make_map(ax):
    # 打开边框
    ax.spines['geo'].set_visible(True)

    # 设置cfeature自带底图
    ax.add_feature(cfeature.COASTLINE.with_scale('50m'), linewidth=1)
    ax.add_feature(cfeature.RIVERS.with_scale('50m'), linewidth=1, zorder=2)

    # 设置shp文件
    shp_path1 = r'E:\SHP\gadm36_CHN_shp\gadm36_CHN_1.shp'
    shp_path2 = r'E:/SHP/world_shp/world_adm0_Project.shp'

    provinces = cfeature.ShapelyFeature(
        Reader(shp_path1).geometries(),
        ccrs.PlateCarree(),
        edgecolor='k',
        facecolor='none')

    world = cfeature.ShapelyFeature(
        Reader(shp_path2).geometries(),
        ccrs.PlateCarree(),
        edgecolor='k',
        facecolor='none')
    ax.add_feature(provinces, linewidth=1.2, zorder=3)
    ax.add_feature(world, linewidth=1.2, zorder=3)

    ax.xaxis.set_major_formatter(LongitudeFormatter())
    ax.yaxis.set_major_formatter(LatitudeFormatter())

    yticks = np.arange(42.5, 55, 5)
    xticks = np.arange(102.5, 130, 5)

    ax.set_yticks(yticks)
    ax.set_xticks(xticks)  # 需要显示的纬度
    ax.tick_params(labelsize=15)

    # 区域
    region = [100.25, 124.25, 40.25, 54.75]
    ax.set_extent(region, crs=ccrs.PlateCarree())

    # 设置网格点
    ylocs = list(yticks)
    xlocs = list(xticks)
    lb = ax.gridlines(draw_labels=True, ylocs=ylocs, xlocs=xlocs, x_inline=False, y_inline=False,
                      linewidth=1, color='lightgray', linestyle='--', alpha=0.8)  # alpha是透明度

    lb.top_labels = False
    lb.bottom_labels = False
    lb.right_labels = False
    lb.left_labels = False
    lb.xlabel_style = {'size': 15}  # 修改经纬度字体大小
    lb.ylabel_style = {'size': 15}

    return ax


def Creat_figure(data1, data2, data3, data4, levels, cmap, lat, lon, title):
    fig = plt.figure(figsize=(16, 4), dpi=1080)
    proj = ccrs.PlateCarree()

    fig.subplots_adjust(left=0.12, bottom=0.12, right=0.88,
                        top=0.9, wspace=0.22, hspace=0.27)

    lon, lat = np.meshgrid(lon, lat)
    title_str = ["(a) 1981-1999 Ave", "(b) 2000-2020 Ave", "(c) DIFF(b-a)"]
    for i in range(1, 4):
        ax = fig.add_subplot(1, 3, i, projection=proj)
        ax = make_map(ax)
        ax.set_title(f"{title_str[i-1]}", fontsize=15, loc="left")
        lcc2 = np.ma.array(lcc, mask=lcc != 200)
        ax.contourf(lon, lat, lcc2,
                    transform=ccrs.PlateCarree(),
                    colors="lightgray", zorder=2, corner_mask=False)
        if i == 1:
            cs = ax.contourf(lon, lat, data1,
                             levels=levels, transform=ccrs.PlateCarree(),
                             cmap=cmap, extend="both", zorder=1, corner_mask=False)
        elif i == 2:
            cs = ax.contourf(lon, lat, data2,
                             levels=levels, transform=ccrs.PlateCarree(),
                             cmap=cmap, extend="both", zorder=1, corner_mask=False)
        else:
            levels2 = np.arange(-60, 60.1, 5)
            cmap2 = cmaps.MPL_bwr_r[15:113]
            cs2 = ax.contourf(lon, lat, data3,
                              levels=levels2, transform=ccrs.PlateCarree(),
                              cmap=cmap2, extend="both", zorder=1, corner_mask=False)
            data4 = np.ma.array(data4, mask=lcc == 200)
            cs3 = ax.contourf(lon, lat, data4,
                              levels=[0, 0.05, 1], transform=ccrs.PlateCarree(),
                              hatches=['..', None], colors="none", zorder=2)

    ######
    cbar_ax = fig.add_axes([0.23, 0.06, 0.3, 0.04])
    cb = fig.colorbar(cs, cax=cbar_ax, orientation='horizontal')
    cb.ax.tick_params(labelsize=15)
    cb.set_label('Units: kg/m2/GSL', fontsize=15)
    ######
    cbar_ax = fig.add_axes([0.675, 0.06, 0.2, 0.04])
    cb = fig.colorbar(cs2, cax=cbar_ax, orientation='horizontal')
    cb.ax.tick_params(labelsize=15)
    cb.set_label('Units: kg/m2/GSL', fontsize=15)

    plt.suptitle(f'{title}', fontsize=20)
    plt.savefig(
        rf'E:/GLDAS Noah/JPG_MG/{title}.jpg', bbox_inches='tight')
    plt.show()


# %%
def PLOT(data1, data2, data3, data4):

    for i in range(3):
        exec(f"print('5 percentile is: ', np.nanpercentile(data{i+1}[0], 5))")
        exec(f"print('95 percentile is: ', np.nanpercentile(data{i+1}[0], 95))")
        print()

    levels = np.arange(0, 271, 30)
    cmap = cmaps.MPL_bwr_r[66:113]

    for l in range(0, 1):
        da1 = data1[l, :, :]
        da2 = data2[l, ]
        da3 = data3[l, ]
        da4 = data4[l, ]
        title = rf"SM GSL(AVE) Diff Layer{l+1}"
        Creat_figure(da1, da2, da3, da4, levels, cmap, lat, lon, title)

# %%


def PLOT2(data1, data2, data3, data4):

    for i in range(1):
        exec(f"print('5 percentile is: ', np.nanpercentile(data{i+1}, 5))")
        exec(f"print('95 percentile is: ', np.nanpercentile(data{i+1}, 95))")
        print()

    levels = np.arange(-35, 35.1, 2)
    cmap = cmaps.MPL_bwr_r[15:113]

    for l in range(4):
        da1 = data1[l, :, :]
        da2 = data2[l, ]
        da3 = data3[l, ]
        da4 = data4[l, ]
        title = rf"SM ANOM GSL(AVE) Diff Layer{l+1}"
        # Creat_figure(da1, da2, da3, da4, levels, cmap, lat, lon, title)


# %%
def PLOT3(data1, data2, data3, data4):

    for i in range(3):
        exec(f"print('5 percentile is: ', np.nanpercentile(data{i+1}, 5))")
        exec(f"print('95 percentile is: ', np.nanpercentile(data{i+1}, 95))")
        print()

    levels = np.arange(-0.8, 0.81, 0.05)
    cmap = cmaps.MPL_bwr_r[15:113]

    for l in range(4):
        da1 = data1[l, :, :]
        da2 = data2[l, ]
        da3 = data3[l, ]
        da4 = data4[l, ]
        title = rf"SM Zscore GSL(AVE) Diff Layer{l+1}"
        Creat_figure(da1, da2, da3, da4, levels, cmap, lat, lon, title)


# %% 生长季平均 sm原值
lcc = read_nc()
sm = read_nc2()

sm1 = sm[:19]  # 81-99
sm2 = sm[19:]  # 00-20

sm1_ave = np.nanmean(sm1, axis=0)
sm2_ave = np.nanmean(sm2, axis=0)
sm_diff = sm2_ave-sm1_ave

_, pt = tTest(sm1, sm2)
PLOT(sm1_ave, sm2_ave, sm_diff, pt)

# %% 生长季平均 sm-异常值
# sm_a = read_nc3()

# sm_a1 = sm_a[:19]  # 81-99
# sm_a2 = sm_a[19:]  # 00-20

# sm_a1_ave = np.nanmean(sm_a1, axis=0)
# sm_a2_ave = np.nanmean(sm_a2, axis=0)
# sma_diff = sm_a2_ave-sm_a1_ave

# _, pt = tTest(sm_a1, sm_a2)
# PLOT2(sm_a1_ave, sm_a2_ave, sma_diff, pt)


# %% 生长季平均 sm-Zscore
# sm_z = read_nc4()

# sm_z1 = sm_z[:19]  # 81-99
# sm_z2 = sm_z[19:]  # 00-20

# smz1_ave = np.nanmean(sm_z1, axis=0)
# smz2_ave = np.nanmean(sm_z2, axis=0)
# smz_diff = smz2_ave-smz1_ave

# _, pt = tTest(sm_z1, sm_z2)
# PLOT3(smz1_ave, smz2_ave, smz_diff, pt)
