...
def Creat_figure(data1, data2, data3, levels, cmap, lat, lon, title):
    ...
    for i in range(1, 4):
        ...
        if i == 1:
            ...
            levels2 = np.arange(-0.5, 0.51, 0.025)
...
def PLOT2(data1, data2, lev):
    ...
    lev2 = lev.split('<')[1]
    title = rf"Drought lai (SPEI under {lev2})"
    # title = rf"Non-Drought lai (SPEI under {lev2})"
    print("\n", title)
...
# %%
lev1 = "(spei)<-1"
gNm1, _, gYm1, gYc1 = lai_drought(lev1, lai1, spei1)
gNm2, _, gYm2, gYc2 = lai_drought(lev1, lai2, spei2)
