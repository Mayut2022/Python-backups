...
def trend(data):
    sp = data.shape[0]
    t = np.arange(sp)
    s, p = np.zeros((180, 300)), np.zeros((180, 300))
...
def Creat_figure(s0, p0, s1, p1, s2, p2, levels, cmap, lat, lon, title):
    ...
    for i in range(1, 4):
        ...
        if i == 1:
            cs = ax.contourf(lon2, lat2, s1,
                             levels=levels, transform=ccrs.PlateCarree(),
                             cmap=cmap, extend="both", zorder=1)
            ax.contourf(lon2, lat2, p1,
                        levels=[0, 0.05, 1], transform=ccrs.PlateCarree(),
                        hatches=['..', None], colors="none", zorder=2)
        elif i == 2:
            cs = ax.contourf(lon2, lat2, s2,
                             levels=levels, transform=ccrs.PlateCarree(),
                             cmap=cmap, extend="both", zorder=1)
            ax.contourf(lon2, lat2, p2,
                        levels=[0, 0.05, 1], transform=ccrs.PlateCarree(),
                        hatches=['..', None], colors="none", zorder=2)
        else:
            cs = ax.contourf(lon2, lat2, s0,
                             levels=levels, transform=ccrs.PlateCarree(),
                             cmap=cmap, extend="both", zorder=1)

            ax.contourf(lon2, lat2, p0,
                        levels=[0, 0.05, 1], transform=ccrs.PlateCarree(),
                        hatches=['..', None], colors="none", zorder=2)
...
# %%
def PLOT(s0, p0, s1, p1, s2, p2):

    print('5 percentile is: ', np.nanpercentile(s2, 5))
    print('95 percentile is: ', np.nanpercentile(s2, 95))

    levels = np.arange(-0.03, 0.031, 0.01)
    cmap = cmaps.MPL_BrBG
    title = r"MG LAI Trend"

    Creat_figure(s0, p0, s1, p1, s2, p2, levels, cmap, lat, lon, title)
