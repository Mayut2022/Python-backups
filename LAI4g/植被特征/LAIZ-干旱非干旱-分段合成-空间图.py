...
def Creat_figure(data1, data2, data3, levels, cmap, lat, lon, title):
    ...
    for i in range(1, 4):
        ...
        if i == 1:
            ...
            levels2 = np.arange(-1.2, 1.21, 0.05)
...
def PLOT2(data1, data2, lev):
    data3 = data2-data1
    levels = np.arange(-0.9, 0.91, 0.05)
    cmap = cmaps.MPL_BrBG[20:109]

    lev2 = lev.split('<')[1]
    
    # title = rf"Non-DroughtZ lai (SPEI under {lev2})"
    title = rf"Drought laiZ (SPEI under {lev2})"
...
# %%
lev1 = "(spei)<-1"
gNm1, _, gYm1, _ = lai_drought(lev1, lai1, spei1)
gNm2, _, gYm2, _ = lai_drought(lev1, lai2, spei2)
