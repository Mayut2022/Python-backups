...
def Creat_figure(data1, levels, cmap, lat, lon, title):
    ...
    for i in range(1, 5):
        ...
        ax.tick_params(labelsize=15)
        if i == 1:
            ax.set_title(f"(a) Wet to Dry Before 1999",
            ...
            ax.set_title(f"(c) Dry to Wet Before 1999",
                         loc="left", fontsize=15)
        elif i == 4:
...
#                  title=f"MG LAI Change Ritio Annual")
...
w2d = (spei_mk > -0.8) & (spei_mk < 0)
yr_w2d = year[w2d]
print("Wet->Dry:", yr_w2d, "\n")

d2w = (spei_mk > 0) & (spei_mk < 0.8)
yr_d2w = year[d2w]
print("Dry->Wet:", yr_d2w, "\n")

# %%
lai_w2d = lai_cr[w2d, :, :]
lai_w2d1 = np.nanmean(lai_w2d[:7, ], axis=0)
lai_w2d2 = np.nanmean(lai_w2d[7:, ], axis=0)

lai_d2w = lai[d2w, :, :]
lai_d2w1 = np.nanmean(lai_d2w[:6, ], axis=0)
lai_d2w2 = np.nanmean(lai_d2w[6:, ], axis=0)
...
def read_plot(data1):
    ...
    Creat_figure(data1, levels, cmap, lat, lon,
                  title=f"MG LAI Change Ritio Annual Atypical")
