...
def plot(data1, data2, title):
    ...
    for i in range(2):
        ...
        ##########################
        if i == 0:
            ax.text(17, 1.7, "2000(drought turning point)", c='b', fontsize=15)
            # 渐变色柱状图
            # 归一化
            ...
            ax.set_title("SPEI03 Annual Diff", loc="left", fontsize=15)
        ##########################
        elif i == 1:
            ax.text(17, 0.25, "2000(drought turning point)", c='b', fontsize=15)
            ax = axes[i]
            # 渐变色柱状图
            # 归一化
            norm = plt.Normalize(-0.25, 0.25)  # 值的范围
            ...
            ax.set_ylim(-0.3, 0.3)
            ax.set_title("LAI Annual Diff", loc="left", fontsize=15)
    # label
...
plot(spei_mk, lai_mk, title=r"Grassland LAI SPEI GSL Annual Diff")
