...
var2 = ["lai1", "lai2", "spei1", "spei2"]
for v in var2:
    exec(f"{v}_re={v}.reshape(39, 1500)")
    exec(f"{v}_re = np.vstack(({v}_re, lcc2))")
    index = pd.Index(list(np.arange(1982, 2021))+["LCC"], name="Year")
...
# %%
data = pd.concat([lai1_df2, lai2_df2, spei1_df2, spei2_df2], axis=1)
data.columns = ["LAI Anom", "LAI Std", "SPEI Ori", "SPEI De"]

#%% 从LAI出发进行筛选， 筛选植被显著增长年份
df_wet = data.copy()
df_wet["LAI Anom"][df_wet["LAI Anom"]==-999]=np.nan

ind = df_wet["LAI Anom"]>=df_wet["LAI Std"]
df_wet["LAI Anom"][~ind] = np.nan
...
df_dry = data.copy()
df_dry["LAI Anom"][df_dry["LAI Anom"]==-999]=np.nan

ind = df_dry["LAI Anom"]<=df_dry["LAI Std"]*-1
df_dry["LAI Anom"][~ind] = np.nan
...
writer = pd.ExcelWriter('LAI Sig Change.xlsx', engine='xlsxwriter')
