...
dfup = pd.read_excel(r"E:/LAI4g/python_MG/抵抗力和恢复力1-2/WUE Sample.xlsx", sheet_name="Sig Up")
dfdn = pd.read_excel(r"E:/LAI4g/python_MG/抵抗力和恢复力1-2/WUE Sample.xlsx", sheet_name="Sig Down")
...
def ttest(df1, df2):
    ...
    for lev in level:
        a = df1["WUE"][df1["Level"]==lev]
        b = df2["WUE"][df2["Level"]==lev]
...
def plot_box(data, title):
    ...
    f = sns.boxplot(ax=ax, data=data, x="Level", y="WUE", hue="State", palette=palette,
                hue_order=["Drought", "Wet"], **kwargs)
    
    x = [str(x).zfill(2) for x in range(7)]
    y = np.zeros(7)
    ax.scatter(x, y-0.02, marker="*", c=mark_test, s=40)
    ax.scatter(x, y-0.04, marker="D", c=markWRS, s=20)
    ...
    ax.set_ylabel("WUE (Units: m2 month/m2 mm)", fontsize=15)
...
def plot_bar(data, title):
    ...
    sns.barplot(data=data, x="Level", y="WUE", 
                hue="State", hue_order=["Drought", "Wet"], palette=palette,
                **kwargs)
...
dfup_2_cut = cut(dfup_2, bin1, "Wet")  ## 显著上升
...
plot_box(df2_plot, title=r"WUE 2000-2020 Boxplot")

#%%
# df2_group = df2_plot["WUE"].groupby([df2_plot['State'], df2_plot['Level']])
...
#%%  转折前1982-1999
bin1 = np.arange(1, 2.76, 0.25)
dfup_1_cut = cut(dfup_1, bin1, "Wet")  ## 显著上升
...
plot_box(df1_plot, title=r"WUE 1982-1999 Boxplot")

# #%%
# df1_group = df1_plot["WUE"].groupby([df1_plot['State'], df1_plot['Level']])
