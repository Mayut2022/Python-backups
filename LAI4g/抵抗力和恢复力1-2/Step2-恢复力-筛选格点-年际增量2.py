...
var2 = ["lai", "lai_std", "spei", "spei_std"]
for v in var2:
...
# # %%
data = pd.concat([lai_df2, lai_std_df2, spei_df2, spei_std_df2], axis=1)
data.columns = ["LAI Diff", "LAI Std", "SPEI Diff", "SPEI Std"]

#%% 从SPEI出发进行筛选，再筛选植被显著变好
df_wet = data.copy()
df_wet["LAI Diff"][df_wet["LAI Diff"]==-999]=np.nan

ind = df_wet["SPEI Diff"]>=df_wet["SPEI Std"]
df_wet["SPEI Diff"][~ind] = np.nan


ind2 = abs(df_wet["LAI Diff"])>=df_wet["LAI Std"]
df_wet["LAI Diff"][~ind2] = np.nan

df_wet = df_wet.dropna(axis=0, how="any")
df_wet.reset_index(inplace=True)

#%% 从SPEI出发进行筛选， 再筛选植被显著变差
df_dry = data.copy()
df_dry["LAI Diff"][df_dry["LAI Diff"]==-999]=np.nan

ind = df_dry["SPEI Diff"]<=df_dry["SPEI Std"]*-1
df_dry["SPEI Diff"][~ind] = np.nan

ind2 = abs(df_dry["LAI Diff"])>=df_dry["LAI Std"]
df_dry["LAI Diff"][~ind2] = np.nan

df_dry = df_dry.dropna(axis=0, how="any")
