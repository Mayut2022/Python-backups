...
def ttest(df1, df2):
    marker = []
    level = [str(x).zfill(2) for x in range(4)]
    for lev in level:
...
def plot_box(data, title):
    ...
    # y = np.zeros(7)
    x = [str(x).zfill(2) for x in range(4)]
    y = np.zeros(4)
    ...
    plt.savefig(rf'E:/LAI4g/python_MG/抵抗力和恢复力1-2/JPG2/{title}.jpg',
                bbox_inches='tight')
...
# plot_box(df2_plot, title=r"0.5 2000-2020 Boxplot")
...
# plot_bar(df2_count, title=r"0.5 2000-2020 Sample Counts")
...
plot_box(df1_plot, title=r"0.5 1982-1999 Boxplot")
...
plot_bar(df1_count, title=r"0.5 1982-1999 Sample Counts")
