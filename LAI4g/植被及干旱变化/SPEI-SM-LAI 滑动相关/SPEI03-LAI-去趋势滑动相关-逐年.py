...
def read_nc2():
    ...
    with nc.Dataset(inpath) as f:
        # print(f.variables.keys())
        spei = (f.variables['spei'][4:9, 1:, ])
        spei_gsl = np.nanmean(spei, axis=0)
...
def plot_Grass(data1, win, sig, title):
    ...
    wh, w = int(win/2), win-1 ####1/2滑动窗口，滑动窗口
    _ = [np.nan]*wh
    _ = np.array(_)
    data1= np.hstack((data1, _))

    fig = plt.figure(1, figsize=(6, 4), dpi=500)
    ...
    ax.set_yticks(np.arange(0.2, 0.76, 0.15))
    # ax.set_xlim(-10, 205)
    # ax.set_xticks(t[::5])
...
# CreatNC(win, r)
r_g = mask(130, r)
print("草地 LAI-SPEI03 年尺度 ", np.nanmax(r_g), np.nanmin(r_g))

plot_Grass(r_g, win, sig, title=rf"相关 LAI-SPEI03 (Win={win}yr) ")
