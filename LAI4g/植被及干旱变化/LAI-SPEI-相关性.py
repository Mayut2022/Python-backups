...
def CreatMap(data1, data2, lon, lat, levels, cmap, title):
    fig = plt.figure(figsize=(5, 4), dpi=1080)
    ...
    cs2 = ax.contourf(lon, lat, data2_mk,
    ...
    ax.text(100.5, 53.5, "(a)", transform=ccrs.PlateCarree(), fontsize=15)
    
    # cbar_ax = fig.add_axes([0.25, 0.08, 0.5, 0.03])
    # cb = fig.colorbar(cs, cax=cbar_ax, orientation='horizontal')
    
    cbar_ax = fig.add_axes([0.95, 0.275, 0.025, 0.5])
    ...
    #             bbox_inches='tight')
    plt.savefig(
        rf'F:/000000 论文图表-正文/{title}.jpg', bbox_inches='tight')
    plt.show()
...
def plot(data1, data2, i):
    ...
    levels = np.arange(0, 0.76, 0.05)
    # cmap = cmaps.MPL_bwr_r[15:113]
    cmap = cmaps.BlueWhiteOrangeRed[162:]
    title = r"SPEI-LAI CORR GSL Mean (Detrend)"
